...
@RocketMQMessageListener(
        ...
        topic = GlobalConstant.Namespace.NAMESPACE + "%" + MqConstant.Topic.USER_DELAY_TOPIC)
public class DelayMsgListener implements RocketMQListener<String> {

    @Override
    public void onMessage(String message) {
        try {
            log.info("<----延迟消费消息---->消息体:【{}】", JSON.toJSONString(message));
        } catch (Exception e) {
            e.printStackTrace();
            log.error("NormalMsgListener 延迟消费消息异常：{}--{}", message, e.getMessage());
        }
    }
}
